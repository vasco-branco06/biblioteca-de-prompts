...
3. Se nenhum gatilho corresponder com clareza, não uses template nenhum e diz
   ...
   escolher nenhum.
4. Um pedido para apagar, substituir ou desfazer alguma coisa não é um gatilho.
   Não dispara template nenhum e ativa a regra de autorização das regras
   permanentes, mesmo que ela ainda não esteja no `CLAUDE.md` do projeto.
5. Verifica o `nao_usar_quando` antes de avançares. Se alguma condição se
   aplicar, descarta e procura outro.
6. Um template de modo `diagnostico` nunca altera ficheiros, mesmo que o pedido
   pareça pedir a correção. Diagnostica, reporta, e pergunta se queres que
   corrija.
7. Preenche as variáveis a partir do contexto da conversa e do projeto. Declara
   as assunções numa linha. Pergunta só quando a dúvida muda mesmo o resultado.
8. Anuncia antes de trabalhar, numa linha e sem cerimónia.
9. Segue o texto do template como instrução tua. Nunca o coles na resposta como
   se fosse conteúdo.
10. Regista o uso no fim.
11. Templates de modo `regra` nunca se executam. Se um for relevante, propõe
...
| PLA-02 | Entrevista antes de escrever seja o que for | quero começar um projeto novo; tenho uma ideia para uma app; vou fazer uma app para; quero construir uma aplicação de raiz; quero fazer um site para; por onde é que começo; ajuda-me a montar isto do zero | execucao |
...
| DIA-01 | Parar e diagnosticar, sem mexer em mais nada | para, não mexas em mais nada; isto está a piorar; não mexas mais; estás a fazer coisas que eu não pedi; já não percebo o que se está a passar aqui | diagnostico |
| DIA-02 | Encontrar a alteração que partiu isto | isto ontem funcionava e hoje não; deixou de funcionar e não sei o que mudou; isto funcionava na semana passada; o que é que estragou isto; estava tudo bem até há pouco; alguma coisa que mexemos partiu outra coisa | diagnostico |
...
| DIA-07 | Recuperar um ficheiro apagado | apaguei um ficheiro sem querer; desapareceu um ficheiro; onde é que foi parar aquele ficheiro; apaguei sem querer a pasta errada; preciso de recuperar uma coisa que apaguei | execucao |
...
| TES-01 | Verificação rápida antes de publicar | quero publicar amanhã; vou pôr isto no ar; vamos lançar esta semana; antes de publicar; como é que sei que não parti nada; vou lançar isto hoje | execucao |
...
| TES-05 | Primeiro um teste que falha, só depois a correção | este erro já tinha aparecido antes; corrigimos isto e voltou; quero garantir que este problema não volta; encontrei um defeito; isto tem de ficar resolvido de vez | execucao |
| TES-06 | Testar com muitos dados, não com três exemplos | quantos utilizadores é que isto aguenta; com poucos dados funciona bem; e se tiver mil registos; aguenta muitas pessoas ao mesmo tempo; isto aguenta o lançamento; não sei se isto escala | execucao |
| DOC-01 | Explica em linguagem simples | explica-me lá o que fizeste aí; não percebi nada do que disseste; não percebi o que mudaste; o que é isso que acabaste de dizer; fala como se eu não soubesse programar; explica isso por palavras simples | diagnostico |
...
| CTL-02 | Rever o próprio trabalho com olhos de cético | revê o que acabaste de fazer; será que isso está mesmo bem; dá uma vista de olhos crítica a isso; antes de eu aprovar isto; vê se não deixaste nada mal feito | execucao |
...
| MKT-02 | Copy de anúncio com vários ângulos e variantes | preciso de um anúncio; escreve-me um anúncio para o Instagram; faz-me um texto para promover isto; preciso de texto para uma campanha paga; não sei o que hei de escrever no anúncio; quero testar várias versões do mesmo anúncio | execucao |
...
| EST-01 | Resumo estruturado de um documento longo | tenho um pdf de oitenta páginas para estudar; preciso de perceber isto até quinta; resume-me este documento; tenho um documento enorme para ler; não tenho tempo para ler isto tudo; o que é que interessa mesmo neste relatório | execucao |
...
| DAD-01 | Diagnóstico de um ficheiro de dados | este excel tem valores esquisitos; as contas não batem certo; a folha de cálculo tem números estranhos; recebi um ficheiro e não sei se posso confiar nele; há linhas repetidas nesta folha; antes de analisar isto quero saber se está bom | diagnostico |
